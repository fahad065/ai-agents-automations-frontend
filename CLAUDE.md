...
`pricing-page.tsx`'s filter tabs (`FILTER_TABS_EN`/`_AR`) gained a **"Chatbots"** tab alongside All/Industries/AI Agents/Automations. It's a separate rendering branch, not just another `moduleType` filter on the existing module list — chatbot pricing varies per template (see backend CLAUDE.md's "Chatbot plan catalog"), so it can't reuse `PricingCard` (which reads `module.pricing.monthly`, unused for chatbots) or show one flat set of plans. Selecting it fetches every chatbot module (`modules.filter(m => m.moduleType === "chatbot")`) and every plan across every template (unfiltered `GET /chatbot-plans`), then renders one `ChatbotTemplateCard` per template — each showing that template's own cheapest non-custom tier as its "From $X/mo" starting price (`basicPlanFor(templateSlug)`, picks the lowest `monthlyFee` among non-`isCustom` plans for that template). The Monthly/Annual billing toggle is hidden for this tab since chatbot plans have no annual rate. Each card's CTA is "View all plans →" linking to `/chatbots/[slug]#pricing` — the full Basic/Pro/Enterprise breakdown and the actual `POST /chatbots` + plan-selection flow live on the template's own detail page, not here. The bottom "Need a custom plan?" CTA was pulled out to render once regardless of which filter tab is active — it used to be nested inside the module-only render branch.
