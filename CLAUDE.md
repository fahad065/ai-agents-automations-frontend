...
`pricing-page.tsx`'s filter tabs (`FILTER_TABS_EN`/`_AR`) gained a **"Chatbots"** tab alongside All/Industries/AI Agents/Automations. It's a separate rendering branch, not just another `moduleType` filter on the existing module list — chatbot pricing is global (Basic/Pro/Enterprise) rather than per-module, so it can't reuse `PricingCard` (which reads `module.pricing.monthly`). Selecting it fetches `GET /chatbot-plans` and renders a new `ChatbotPlanCard` per plan instead; the Monthly/Annual billing toggle is hidden for this tab since chatbot plans have no annual rate. Each plan card's CTA is "Choose a template →" linking to `/chatbots`, not a direct signup/create action — picking a specific template is required before a chatbot can actually be created (see `chatbot-detail-page.tsx`'s pricing section, which is where the real `POST /chatbots` + plan selection happens). The bottom "Need a custom plan?" CTA was pulled out to render once regardless of which filter tab is active — it used to be nested inside the module-only render branch.
