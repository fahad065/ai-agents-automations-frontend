      ...
      <div onClick={(e) => e.stopPropagation()} style={{
          ...
          <div style={{ display: "flex", gap: "2px", marginTop: "16px", background: colors.bg, borderRadius: "8px", padding: "3px" }}>
            {(["overview", "setup"] as const).map((s) => (
              <button key={s} onClick={() => setStep(s)} style={{
                ...
                {s === "overview" ? "Overview" : "Setup & Configure"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "20px 24px" }}>
          ...
          {step === "overview" && (
            <div>
              {module.tagline && (
                <p style={{ fontSize: "14px", color: colors.textMuted, lineHeight: 1.7, marginBottom: "20px" }}>
                  {module.tagline}
                </p>
              )}
              ...
              {module.requiredApiKeys?.length > 0 && (
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                    ...
                    <p style={{ fontSize: "13px", fontWeight: 600, color: colors.text }}>Required API Keys</p>
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                    {module.requiredApiKeys.map((k) => (
                      <span key={k} style={{
              ...
              <div style={{
                ...
                {module.estimatedCostPerRun && (
                  <p style={{ fontSize: "12px", color: colors.textMuted }}>
                    + ~{module.estimatedCostPerRun} in API costs per run
                  </p>
                )}
                <p style={{ fontSize: "12px", color: "#22c55e", marginTop: "6px", fontWeight: 500 }}>
          ...
          {step === "setup" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              <div>
                ...
                <input value={form.niche}
                  onChange={(e) => setForm(f => ({ ...f, niche: e.target.value }))}
                  style={inp} placeholder="e.g. Dark psychology and human behavior" />
                <p style={{ fontSize: "11px", color: colors.textMuted, marginTop: "4px" }}>
                  The AI will research topics and create content around this niche.
                </p>
              </div>
              ...
              {needsInstagram && (
                <div style={{
                  ...
                  <input value={form.instagramAccountId}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <div>
                  ...
                  <select value={form.scheduleFrequency}
              ...
              {error && (
                <p style={{ fontSize: "12px", color: "#ef4444", padding: "8px 12px", background: "rgba(239,68,68,0.08)", borderRadius: "7px" }}>
                  {error}
                </p>
              )}
            </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: `1px solid ${panelBorder}`, display: "flex", gap: "10px" }}>
          {step === "overview" ? (
            ...
            <>
              ...
              <button onClick={handleSubscribe} disabled={saving || !form.niche.trim()} style={{
                flex: 2, padding: "10px", borderRadius: "8px",
                cursor: saving || !form.niche.trim() ? "not-allowed" : "pointer",
                background: "linear-gradient(135deg, #7c3aed, #6d28d9)",
                color: "white", border: "none", fontSize: "13px", fontWeight: 600,
                display: "flex", alignItems: "center", justifyContent: "center", gap: "8px",
                opacity: saving || !form.niche.trim() ? 0.7 : 1,
              }}>
...
function MarketplaceModal({ onClose, onSubscribed, colors, isDark }: {
  ...
  useEffect(() => {
    api.get("/modules?limit=50").then(res => {
  ...
  if (selected) {
    return (
      <SubscribeModal
        ...
        onSuccess={() => { setSelected(null); onClose(); onSubscribed(); }}
        colors={colors}
        isDark={isDark}
      />
    );
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        background: panelBg, border: `1px solid ${panelBorder}`,
        borderRadius: "18px", width: "100%", maxWidth: "700px",
        maxHeight: "90vh", display: "flex", flexDirection: "column",
        boxShadow: "0 32px 80px rgba(0,0,0,0.5)",
      }}>
        {/* Header */}
              ...
              {filtered.map((m) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                    <div style={{
                      width: "38px", height: "38px", borderRadius: "9px", fontSize: "20px",
                  ...
                  {m.tagline && (
                    <p style={{ fontSize: "12px", color: isDark ? "#737373" : "#6b7280", lineHeight: 1.5, marginBottom: "10px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden" }}>
                      {m.tagline}
                    </p>
                  )}
...
export function MyModulesPage() {
  ...
  const toggleModule = async (id: string) => {
    try {
      await api.patch(`/usermodules/${id}/toggle`);
      fetchModules();
    } catch {}
  };
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <p style={{ color: colors.textMuted, fontSize: "14px", marginBottom: "20px" }}>
            {search ? "Try a different search term." : "Browse the marketplace to add your first module."}
          </p>
          {!search && (
            <button onClick={() => setShowMarketplace(true)} style={{
