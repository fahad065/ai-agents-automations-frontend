...
function TemplateModal({ template, nextSortOrder, onClose, onSave, colors, isDark }: {
  ...
  const [nicheModules, setNicheModules] = useState<AgentTemplate[]>([]);
  ...
  const [form, setForm] = useState({
    ...
    pipelineCategory: template?.pipelineCategory || "standalone",
    nicheSlug: template?.nicheSlug || "content_social",
    availableIn: template?.availableIn || ["UAE"] as string[],
    componentSlugs: (template?.components || []).map((c: any) => typeof c === 'string' ? c : c.key) as string[],
  });

  // Fetch standalone modules for the selected niche when niche_pipeline is active
  useEffect(() => {
    if ((form as any).pipelineCategory !== 'niche_pipeline') return;
    api.get(`/modules?nicheSlug=${(form as any).nicheSlug}&pipelineCategory=standalone&limit=50`)
      .then(res => setNicheModules(res.data?.data || res.data || []))
      .catch(() => setNicheModules([]));
  }, [(form as any).nicheSlug, (form as any).pipelineCategory]);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        pipelineCategory: (form as any).pipelineCategory,
        nicheSlug: (form as any).nicheSlug,
        availableIn: (form as any).availableIn,
        components: (form as any).componentSlugs.map((slug: string) => {
          const mod = nicheModules.find(m => m.slug === slug);
          return mod
            ? { key: mod.slug, name: mod.name, description: mod.tagline || mod.description || '', icon: mod.icon, isRequired: true, sortOrder: mod.sortOrder }
            : { key: slug, name: slug, description: '', icon: '🔧', isRequired: true, sortOrder: 99 };
        }),
      };
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <div>
                  ...
                  <select
                    value={(form as any).pipelineCategory}
                    onChange={(e) => setForm(f => ({ ...f, pipelineCategory: e.target.value }))}
                    style={inp}
                  >
                ...
                <div>
                  {lbl("Niche")}
                  <select
                    value={(form as any).nicheSlug}
                    onChange={(e) => setForm(f => ({ ...f, nicheSlug: e.target.value }))}
                    style={inp}
                  >
                ...
                <div style={{ display: "flex", gap: "16px" }}>
                  {COUNTRY_OPTIONS.map(c => (
                    <label key={c} style={{ display: "flex", alignItems: "center", gap: "6px", cursor: "pointer", fontSize: "13px", color: colors.text }}>
                      <input
                        type="checkbox"
                        checked={(form as any).availableIn.includes(c)}
                        onChange={() => setForm(f => {
                          const cur = (f as any).availableIn as string[];
                          return { ...f, availableIn: cur.includes(c) ? cur.filter((x: string) => x !== c) : [...cur, c] };
                        })}
              ...
              {(form as any).pipelineCategory === "niche_pipeline" && (
                <div>
                  {lbl("Pipeline Components", `— select standalone agents from the ${NICHE_OPTIONS.find(n => n.value === (form as any).nicheSlug)?.label || ''} niche`)}
                  {nicheModules.length === 0 ? (
                    <p style={{ fontSize: "12px", color: colors.textMuted, padding: "10px", background: colors.bg, borderRadius: "8px", border: `1px solid ${colors.border}` }}>
                      No standalone agents found for this niche yet. Create standalone agents in this niche first.
                    </p>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      {nicheModules.map(mod => {
                        const selected = (form as any).componentSlugs.includes(mod.slug);
                        return (
                          <div key={mod.slug} style={{
                            display: "flex", alignItems: "center", justifyContent: "space-between",
                            padding: "10px 12px", borderRadius: "8px", cursor: "pointer",
                            border: `1px solid ${selected ? "rgba(124,58,237,0.35)" : colors.border}`,
                            background: selected ? "rgba(124,58,237,0.06)" : colors.bg,
                            transition: "all 0.15s",
                          }}
                            onClick={() => setForm(f => {
                              const cur = (f as any).componentSlugs as string[];
                              return { ...f, componentSlugs: cur.includes(mod.slug) ? cur.filter((s: string) => s !== mod.slug) : [...cur, mod.slug] };
                            })}
                          >
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              ...
                              <div>
                                <p style={{ fontSize: "13px", fontWeight: selected ? 600 : 400, color: selected ? "#a78bfa" : colors.text }}>{mod.name}</p>
                                <p style={{ fontSize: "11px", color: colors.textMuted }}>{mod.tagline || mod.description?.slice(0, 60)}</p>
                              </div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                              ...
                              <div style={{ position: "relative" }}>
                                <button
                                  onClick={(e) => { e.stopPropagation(); setTooltipSlug(tooltipSlug === mod.slug ? null : mod.slug); }}
                                  style={{ width: "20px", height: "20px", borderRadius: "50%", border: `1px solid ${colors.border}`, background: colors.bg, cursor: "pointer", fontSize: "11px", color: colors.textMuted, display: "flex", alignItems: "center", justifyContent: "center" }}
                                >ⓘ</button>
                                {tooltipSlug === mod.slug && (
                                  <div style={{
                                    position: "absolute", right: 0, bottom: "26px", zIndex: 10,
                                    background: isDark ? "#1a1a1a" : "#fff",
                                    border: `1px solid ${colors.border}`, borderRadius: "8px",
                                    padding: "10px 12px", width: "240px",
                                    boxShadow: "0 8px 24px rgba(0,0,0,0.2)",
                                  }}>
                                    <p style={{ fontSize: "12px", fontWeight: 600, color: colors.text, marginBottom: "4px" }}>{mod.name}</p>
                                    <p style={{ fontSize: "11px", color: colors.textMuted, lineHeight: 1.5 }}>{mod.description?.slice(0, 140)}</p>
                                    {mod.estimatedCostPerRun && <p style={{ fontSize: "11px", color: "#22c55e", marginTop: "4px" }}>~{mod.estimatedCostPerRun}</p>}
                                  </div>
                                )}
                              </div>
                              {/* checkbox */}
                  ...
                  {(form as any).componentSlugs.length > 0 && (
                    <p style={{ fontSize: "11px", color: "#a78bfa", marginTop: "6px" }}>
                      {(form as any).componentSlugs.length} component{(form as any).componentSlugs.length > 1 ? 's' : ''} selected
                    </p>
                  )}
                </div>
              )}
